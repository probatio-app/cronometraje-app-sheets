    ...
    if (selectionSummary) {
        selectionSummary.addEventListener('click', () => {
            ...
            if (isCollapsed) {
                ...
                selectionDropdowns.classList.remove('collapsed');
                expandBtn.textContent = '▼';
            }
        });
    }
    
    // Actualizar summary cuando cambian las selecciones
    const updateSummary = () => {
        const club = selectedClub || '--';
        const division = selectedDivision || '--';
        const test = selectedTest || '--';
        summaryText.textContent = `${club} / ${division} / ${test}`;
        
        // Mostrar summary solo si hay al menos una selección
...
const addDivision = async (club, divisionName) => {
   ...
   const addAthlete = async (club, division, athleteName) => {
    ...
    const showTestedResults = () => {
        ...
        const results = getCurrentTestedResults();
        const config = testConfig[selectedTest];
        const isCombiTest = config && config.times > 1 && config.repetitions > 1;
        
        // Usar el mismo HTML que showResults
        let headerHtml;
        if (isCombiTest) {
            headerHtml = `
                <div class="table-header">
        ...
        // Mostrar resultados igual que showResults
        if (results.length === 0) {
            resultsTable.innerHTML += '<div class="no-results">No results available yet</div>';
        } else {
            results.forEach((result, index) => {
                const row = document.createElement('div');
                row.className = `athlete-row ${index % 2 === 0 ? 'even' : 'odd'}`;
                ...
                if (result.athlete.includes('REP')) {
                    const parts = result.athlete.split(' ');
                    athleteName = parts[0].replace('-DNS', ''); 
                    repNumber = parts[1].replace('REP', '');
                } else if (result.athlete.includes('SPLIT')) {
                    athleteName = result.athlete.replace(' SPLIT', '');
                } else if (result.athlete.includes('LAP')) {
                    athleteName = result.athlete.replace(' LAP', '');
                ...
                if (isCombiTest) {
                    row.innerHTML = `
                        <div class="athlete-cell position" style="color: #ccc">${result.position || ''}</div>
                        <div class="athlete-cell name" style="color: #ccc">${showName ? athleteName : ''}</div>
                        <div class="athlete-cell rep" style="color: #ccc">${showRep}</div>
                        <div class="athlete-cell type" style="color: ${typeText === 'SPLIT' ? '#4ecdc4' : '#ffa500'}">${typeText}</div>
                        ${result.athlete.includes('LAP') ? result.laps : result.splits}
                    `;
                } else {
                    row.innerHTML = `
                        <div class="athlete-cell position" style="color: #ccc">${result.position || ''}</div>
    ...
    const getCurrentTestedResults = () => {
        ...
        // Combinar atletas testeados con los no testeados como DNS
        const allResults = [...testedAthletes];
        ...
        // Procesar y formatear resultados igual que getCurrentResults
        const results = [];
        
        if (isCombiTest) {
            allResults.forEach((result) => {
                if (result.isCombiTest && result.allRepetitions) {
                    result.allRepetitions.forEach((rep) => {
                        // Fila SPLIT
                        results.push({
                            athlete: `${result.name} REP${rep.repetition} SPLIT`,
                            time: result.formattedTime,
                            splits: rep.splits.map((val, idx) => 
                                `<div class="athlete-cell split ${idx === 5 ? 'no-border' : ''}" style="color: ${val === 'DNS' ? '#ff6b6b' : val === '--' || val === '' ? '#666' : '#4ecdc4'}">${val}</div>`
                            ).join(''),
                            laps: '',
                        ...
                        results.push({
                            ...
                            time: '',
                            splits: '',
                            laps: rep.laps.map((val, idx) => 
                                `<div class="athlete-cell split ${idx === 5 ? 'no-border' : ''}" style="color: ${val === 'DNS' ? '#ff6b6b' : val === '--' || val === '' ? '#666' : '#ffa500'}">${val}</div>`
                            ).join(''),
                            rawTime: result.totalTime,
                            repNumber: rep.repetition
                        });
            ...
            allResults.forEach((result) => {
                ...
                if (typeof result === 'string') {
                    ...
                    athleteData = {
                        ...
                        laps: result.laps || ['--', '--', '--', '--', '--', '--'],
                        rawTime: result.status === 'DNS' ? Infinity : result.time
                    };
                }
                ...
                results.push({
                    ...
                    splits: athleteData.splits.map((val, idx) => 
                        `<div class="athlete-cell split ${idx === 5 ? 'no-border' : ''}" style="color: ${val === 'DNS' ? '#ff6b6b' : val === '--' || val === '' ? '#666' : '#4ecdc4'}">${val}</div>`
                    ).join(''),
                    laps: '',
                    rawTime: athleteData.rawTime,
                    showSplits: true
                });
                ...
                results.push({
                    ...
                    splits: '',
                    laps: athleteData.laps.map((val, idx) => 
                        `<div class="athlete-cell split ${idx === 5 ? 'no-border' : ''}" style="color: ${val === 'DNS' ? '#ff6b6b' : val === '--' || val === '' ? '#666' : '#ffa500'}">${val}</div>`
                    ).join(''),
                    rawTime: athleteData.rawTime,
                    showSplits: false
        ...
        const athleteGroups = [];
        const uniqueAthletes = [...new Set(results.map(r => r.athlete.split(' ')[0]))];
